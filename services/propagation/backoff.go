...
// 30 seconds.
func ComputeBackoff(baseBackoffMs, attempt int) time.Time {
	...
	delay = time.Duration(float64(delay) + (rand.Float64()*2-1)*jitter) //nolint:gosec // math/rand is fine for backoff jitter; not a security boundary
	if delay < 0 {
		delay = 0
	}
