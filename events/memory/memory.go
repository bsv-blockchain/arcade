...
type InMemoryPublisher struct {
	subscribers []chan models.StatusUpdate
	mu          sync.RWMutex
	bufferSize  int
}
...
func NewInMemoryPublisher(bufferSize int) events.Publisher {
	return &InMemoryPublisher{
		subscribers: make([]chan models.StatusUpdate, 0),
		bufferSize:  bufferSize,
	}
}

// Publish sends a status update to all subscribers
func (p *InMemoryPublisher) Publish(ctx context.Context, event models.StatusUpdate) error {
	p.mu.RLock()
	defer p.mu.RUnlock()
...
func (p *InMemoryPublisher) Subscribe(ctx context.Context) (<-chan models.StatusUpdate, error) {
	p.mu.Lock()
	defer p.mu.Unlock()

	ch := make(chan models.StatusUpdate, p.bufferSize)
