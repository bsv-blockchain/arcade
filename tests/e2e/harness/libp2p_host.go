...
// NewLibP2PHost spins up the harness's in-process libp2p peer. Network is
// the BSV-network identifier — pass "regtest" for the smoke test.
//
// listenPort can be 0 to let the OS pick a free port; the chosen port is
// reflected in the returned BootstrapMultiaddr.
//
// On success, the returned host's BootstrapMultiaddr is what gets fed to
// merkle-service via P2P_BOOTSTRAP_PEERS. Close releases the libp2p host
// and waits for goroutines.
func NewLibP2PHost(t *testing.T, network string, listenPort int) (*LibP2PHost, error) {
	t.Helper()
	if network == "" {
		network = teranodep2p.NetworkRegtest
	}
	...
	announceAddr := fmt.Sprintf("/dns4/host.docker.internal/tcp/%d", listenPort)
	...
	// Bootstrap multiaddr the merkle-service container dials — uses the
	// dns4 form because the container resolves host.docker.internal at
	// connect time. The peer ID anchors the dial so libp2p verifies the
	// remote identity.
	bootstrap := fmt.Sprintf("/dns4/host.docker.internal/tcp/%d/p2p/%s", listenPort, peerID)
	...
	for _, topic := range []string{
		teranodep2p.TopicName(network, teranodep2p.TopicBlock),
		teranodep2p.TopicName(network, teranodep2p.TopicSubtree),
	} {
